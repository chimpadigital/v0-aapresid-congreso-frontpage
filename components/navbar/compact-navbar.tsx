...
export function CompactNavbar({ scrolled, menuOpen, toggleMenu, locale }: CompactNavbarProps) {
  return (
    <nav
      ...
      <div className="z-[9999999] mx-auto my-4 max-w-7xl">
        <div className="flex items-center justify-between rounded-full bg-transparent px-4 py-3 md:px-6">
          <div className="flex items-center">
            <Link href="/" className="flex items-center">
              <div className="rounded-full bg-[#7D7777]/70 px-4 py-[13.6px] backdrop-blur-sm">
                <Image src="/images/logos-juntos.svg" alt="Congreso Aapresid" width={220} height={40} />
              </div>
            </Link>
          </div>
          ...
          <div className="z-[9999999] flex items-center gap-3">
            ...
            >
              {menuOpen ? <X className="h-6 w-6 text-primary" /> : <Menu className="h-6 w-6 text-white" />}
            </button>
          </div>
        </div>
      </div>
